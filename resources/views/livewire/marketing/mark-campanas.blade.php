<div>
    ...
    <div class="flex flex-row justify-between">
        <h3 style="font-size: 18px;"> <strong>LISTA DE CAMPAÑAS</strong> </h3>

        @livewire('marketing.crear-campana')
    ...
    <div class="table-responsive">
        <table id="mitablaregistros1" class="table table-striped" role="grid" data-bs-toggle="data-table">
            ...
            <tbody>
                @foreach ($campañas as $item)
                    <tr>
                        <td>{{ $item->tipo }}</td>
                        <td>{{ $item->estado }}</td>
                        <td style="display: flex;">
                            @livewire('marketing.editar-campana', ['idcampana' => $item->id], key($item->id))
                            <a class="ml-3 btn btn-sm btn-icon btn-danger" data-bs-toggle="tooltip"
                                data-bs-placement="top" title="Edit" data-original-title="Edit"
                                wire:click="$emit('inactivarCampana',{{ $item->id }})">
