<div>
    <a class="mr-2 btn btn-sm btn-icon btn-warning" data-bs-toggle="tooltip" data-bs-placement="top" title="Editar"
    ...
    @if ($producto->estado == 'Activo' && (Auth::user()->rol = 'Administrador'))
        <a class="btn btn-sm btn-icon btn-danger" data-bs-toggle="tooltip" data-bs-placement="top" title="Desactivar"
            data-original-title="Edit" wire:click="desactivar">
            <span class="btn-inner">
    ...
    @else
        <a class="btn btn-sm btn-icon btn-success" data-bs-toggle="tooltip" data-bs-placement="top" title="Activar"
            data-original-title="Edit" wire:click="activar">
            <span class="btn-inner">
                <svg class="icon-20" width="20" viewBox="0 0 24 24" fill="none"
                    xmlns="http://www.w3.org/2000/svg">
                    <path opacity="0.4"
    ...
    <x-modal wire:model.defer="editar">
                ...
                <form>
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="exampleInputDisabled1"
                            wire:model.defer="producto.nombre" disabled>
                    </div>
                    <div class="form-group">
                        <label class="form-label" for="">Cantidad inicial de excel:</label>
                        <input type="number" class="form-control" id="exampleInputDisabled1"
                            wire:model.defer="producto.inicio">
                    </div>
                    <div class="form-group">
                        <label class="form-label" for="">Cantidad actual:</label>
                        <input type="number" class="form-control" id="exampleInputDisabled1"
                            wire:model.defer="producto.cantidad">
                    </div>
                    <div class="form-group">
                    ...
                    @if ($image)
                        ...
                        @endif
                    @else
                        <img class="mt-4" src="{{ asset('storage/' . $producto->path) }}" alt="">
                    @endif
        ...
        <div class="flex flex-row justify-end px-6 py-4 text-right bg-gray-100">
            <label type="submit" class="btn btn-success" wire:click="guardaredicion" wire:loading.remove
                wire:target="guardartodo">Guardar</label>
            <span class="" wire:loading wire:target="guardaredicion">Guardando...</span>
        </div>
    </x-modal>
</div>
